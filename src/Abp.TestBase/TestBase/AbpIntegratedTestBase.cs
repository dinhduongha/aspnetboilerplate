...
namespace Abp.TestBase
{
        ...
        where TStartupModule : AbpModule
    {
        ...
        protected virtual void WithUnitOfWork(long? tenantId, Action action, UnitOfWorkOptions options = null)
        {
            using (var uowManager = LocalIocManager.ResolveAsDisposable<IUnitOfWorkManager>())
            {
                using (var uow = uowManager.Object.Begin(options ?? new UnitOfWorkOptions()))
        ...
        protected async Task WithUnitOfWorkAsync(long? tenantId, Func<Task> action, UnitOfWorkOptions options = null)
        {
            using (var uowManager = LocalIocManager.ResolveAsDisposable<IUnitOfWorkManager>())
            {
                using (var uow = uowManager.Object.Begin(options ?? new UnitOfWorkOptions()))
