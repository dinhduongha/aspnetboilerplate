...
namespace Abp.Zero.SampleApp.Tests.Webhooks
{
    public class WebhookTestBase : SampleAppTestBase<WebhookPublishTestModule>
    {
        ...
        protected async Task<long> CreateAndGetTenantIdWithFeaturesAsync(string featureKey, string featureValue)
        {
        ...
        /// </summary>
        /// <param name="tenantFeatures"></param>
        protected async Task<long> CreateAndGetTenantIdWithFeaturesAsync(Dictionary<string, string> tenantFeatures = null)
        {
            string name = Guid.NewGuid().ToString().Replace("-", "");

            var tenant = new Tenant(name, name);
            var tenantId = await Resolve<IRepository<Tenant, long>>().InsertAndGetIdAsync(tenant);
        ...
        protected async Task AddOrReplaceFeatureToTenantAsync(long tenantId, string featureName, string featureValue)
        {
